...
Execute a plan by leading persistent worker teammates: one worker per context domain, task triage before every dispatch, live question-answering while workers work, a fresh review of each worker's whole diff before it shuts down, and a whole-branch review at the end.
...
| Reviewer | One-shot subagent | Sonnet or Haiku ONLY, by diff size and risk — never the session model | Per review (risky task or domain) |
...
At plan read, segment the plan's tasks into **context domains** — groups that share files, interfaces, or knowledge. A domain is the unit of worker context: one worker per domain, spawned at domain start, shut down at domain end after its review is clean and its journal is written.
...
digraph process {
    ...
    "Spawn worker(s) for current domain(s), record domain BASE (./worker-prompt.md)" [shape=box];
    ...
    subgraph cluster_per_task {
        ...
        "Worker reports DONE" [shape=box];
        "Risky tier?" [shape=diamond];
        "Dispatch task reviewer (./reviewer-prompt.md); forward findings verbatim, re-review until clean" [shape=box];
        "Mark task complete in ledger" [shape=box];
    }

    "Domain tasks remain?" [shape=diamond];
    "Sole domain in the plan?" [shape=diamond];
    "Generate domain review package, dispatch domain reviewer (./reviewer-prompt.md)" [shape=box];
    "Dispatch final whole-branch reviewer (Opus) — this IS the sole domain's review" [shape=box];
    "Review clean?" [shape=diamond];
    "Forward findings to worker verbatim, dispatch fresh re-review" [shape=box];
    "Worker finalizes journal, confirm shutdown" [shape=box];
    "Domains remain?" [shape=diamond];
    "Dispatch final whole-branch reviewer (Opus); ONE fix subagent for any findings" [shape=box];
    ...
    "Create feature worktree (superpowers:using-git-worktrees)" -> "Spawn worker(s) for current domain(s), record domain BASE (./worker-prompt.md)";
    "Spawn worker(s) for current domain(s), record domain BASE (./worker-prompt.md)" -> "Triage task";
    ...
    "Worker reports DONE" -> "Risky tier?";
    "Risky tier?" -> "Dispatch task reviewer (./reviewer-prompt.md); forward findings verbatim, re-review until clean" [label="yes"];
    "Risky tier?" -> "Mark task complete in ledger" [label="no"];
    "Dispatch task reviewer (./reviewer-prompt.md); forward findings verbatim, re-review until clean" -> "Mark task complete in ledger";
    ...
    "Domain tasks remain?" -> "Sole domain in the plan?" [label="no"];
    "Sole domain in the plan?" -> "Dispatch final whole-branch reviewer (Opus) — this IS the sole domain's review" [label="yes"];
    "Sole domain in the plan?" -> "Generate domain review package, dispatch domain reviewer (./reviewer-prompt.md)" [label="no"];
    "Dispatch final whole-branch reviewer (Opus) — this IS the sole domain's review" -> "Review clean?";
    "Generate domain review package, dispatch domain reviewer (./reviewer-prompt.md)" -> "Review clean?";
    "Review clean?" -> "Forward findings to worker verbatim, dispatch fresh re-review" [label="no"];
    "Forward findings to worker verbatim, dispatch fresh re-review" -> "Review clean?";
    "Review clean?" -> "Worker finalizes journal, confirm shutdown" [label="yes"];
    "Worker finalizes journal, confirm shutdown" -> "Domains remain?";
    "Domains remain?" -> "Spawn worker(s) for current domain(s), record domain BASE (./worker-prompt.md)" [label="yes"];
    "Domains remain?" -> "Dispatch final whole-branch reviewer (Opus); ONE fix subagent for any findings" [label="no — multiple domains ran"];
    "Domains remain?" -> "superpowers:finishing-a-development-branch" [label="no — sole domain, already reviewed"];
    "Dispatch final whole-branch reviewer (Opus); ONE fix subagent for any findings" -> "superpowers:finishing-a-development-branch";
}
```
...
|---|---|---|
| **Trivial** | Config tweak, rename, one-liner where the plan contains the exact change | You implement it inline and commit. No dispatch — the domain and final reviews cover it. |
| **Standard** | Everything else — the default | Task brief to the domain worker. |
| **Risky** | New architecture, shared interfaces, anything you are uncertain about | Worker messages back a short approach plan BEFORE writing code; you approve or correct (escalate if the plan itself is in question); then the standard flow, plus a task review (always Sonnet) on DONE — the only tier reviewed per task. |
...
**DONE:** for a risky task, run its task review first (see Reviews). Then mark the task complete in the ledger and send the next task. After the domain's last task, start the domain review.
...
- The default review unit is the domain: one review of the worker's whole diff after its last task, while the worker is still live to fix findings. Standard and trivial tasks get no per-task review — they ship on the worker's tests and self-review until the domain review.
- Risky tasks are the exception: review each one on its own diff as soon as the worker reports DONE, always on Sonnet.
- Record the commit at domain start as the domain's BASE in the ledger, and the commit before each risky task as that task's BASE — review packages come from `scripts/review-package BASE HEAD` (prints the path it wrote). Never diff from `HEAD~1`; it silently drops everything before the last commit.
- Domain reviewer model is Sonnet or Haiku, scaled to the diff — a subtle concurrency change gets Sonnet; a small mechanical diff gets Haiku. Never the session model.
- The reviewer gets the brief files, the report files, and the review package path, plus the plan's binding Global Constraints copied verbatim. The template carries the process rules; the constraints block is for what THIS project's spec demands.
- Do not pre-judge findings: never tell a reviewer what not to flag or pre-rate a finding's severity. Adjudicate in the loop.
- **Forward findings to the worker VERBATIM.** Never paraphrase, summarize, or soften review findings. The worker fixes, re-runs the covering tests, appends to its report file; then dispatch a fresh re-review. Repeat until clean. The worker shuts down only after its review is clean.
- A worker failing its review through two full cycles means stop: escalate to your human partner with the findings. No third identical attempt.
...
- **Final whole-branch review:** always Opus, on `scripts/review-package MERGE_BASE HEAD` (MERGE_BASE = `git merge-base main HEAD`), using superpowers:requesting-code-review's code-reviewer template. When the plan ran a single domain, this IS that domain's review: run it before the worker shuts down and forward findings to the worker. When multiple domains ran, run it after all workers have shut down; if it returns findings, dispatch ONE fix subagent with the complete findings list — not one fixer per finding.
...
- **Ledger** (`progress.md`) — YOUR memory, the only bookkeeping in your context: the domain map with each domain's BASE commit, one line per completed task (`Task N: complete (commits <base7>..<head7>)`), review outcomes, Minor findings, segmentation errors. At skill start, check for an existing ledger — tasks marked complete are DONE; resume at the first task not marked complete. After compaction or resume, trust the ledger and `git log` over your own recollection.
...
- Dispatch a task or domain reviewer on the session model, or the final review on anything but Opus
- Dispatch a per-task review on a standard or trivial task — only risky tasks and domains get reviewed
...
- Accept a review missing either verdict (spec compliance AND work quality)
...
- Shut a worker down before its review is clean and its journal is finalized
...
- [reviewer-prompt.md](reviewer-prompt.md) — dispatch a reviewer subagent (risky task or whole domain)
